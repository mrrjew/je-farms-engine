...
export class PaymentService {
  ...
  async initializeTransaction(dto: PaymentDto, thisUser:string): Promise<string | null> {
    try {
      ...
      if (!_isUser) {
          throw new UnauthorizedException('Email does not correspond to current user');
        }
      ...
      const response = await paystack.transaction.initialize({
        ...
        name: 'Coconut Farm', // Specify the name of your application or service
      });

      console.log(response.data)
      return response.data.authorization_url; // Return the authorization URL for redirection
    } catch (error) {
      throw new InternalServerErrorException('Paystack initialization error:', error);
    }
  }
  ...
  async verifyTransaction(reference: string): Promise<boolean> {
    try {
      ...
      return false;
        ...
        throw new InternalServerErrorException('Paystack verification error:', error);
